...
//Rare exceptions to indicate a bug in the _livinghappy code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_livinghappy = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_livinghappy]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.livinghappy.module"
